...
ActivityGraph.prototype.amount = function (tx, sign) {
    return (sign + tx.amount / Math.pow(10, 8)) + ' LWF';
};
...
var Header = function ($rootScope) {
    ...
    this.updatePriceTicker = function (res) {
        ...
        // When ticker for user-stored currency is not available - switch to LWF temporarly
        if ($rootScope.currency.symbol !== 'LWF' && (!$rootScope.currency.tickers || !$rootScope.currency.tickers.LWF || !$rootScope.currency.tickers.LWF[$rootScope.currency.symbol])) {
            console.log ('Currency ' + $rootScope.currency.symbol + ' not available, fallback to LWF');
            $rootScope.currency.symbol = 'LWF';
        }
    };
...
angular.module('lisk_explorer')
  ...
  .filter('currency', function (numberFilter, liskFilter) {
      return function (amount, currency, decimal_places) {
        ...
        if (currency.tickers && currency.tickers.LWF && currency.tickers.LWF[currency.symbol]) {
          factor = currency.tickers.LWF[currency.symbol];
        } else if (currency.symbol !== 'LWF') {
