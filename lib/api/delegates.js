...
module.exports = function (app) {
    function Active () {
        this.getActive = function (cb) {
            request.get({
                url : app.get('lisk address') + '/api/delegates/?orderBy=rate:asc&limit=201',
                json : true
        ...
        this.getVoters = function (delegate, cb) {
            request.get({
                url : app.get('lisk address') + '/api/delegates/voters?publicKey=' + delegate.publicKey,
                json : true
            }, function (err, response, body) {
                if (err || response.statusCode !== 200) {
                    return cb(err || 'Response was unsuccessful');
                } else if (body.success === true) {
                    delegate.voters = body.accounts;
                    return cb();
                } else {
                    delegate.voters = [];
                    return cb();
    ...
    this.getActive = function (error, success) {
        ...
        async.waterfall([
            ...
            function (result, cb) {
                async.each(result.delegates, function (delegate, callback) {
                    delegates.getVoters(delegate, callback);
                }, function (err) {
                    if (err) {
                        return cb(err);
                    } else {
    ...
    this.getNextForgers = function (error, success) {
        request.get({
            url : app.get('lisk address') + '/api/delegates/getNextForgers?limit=201',
            json : true
        }, function (err, response, body) {
            if (err || response.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
    ...
    this.getDelegateProposals = function (error, success) {
            var offset = 0,
                limit  = 25,
                nextPage = false, // https://lwf.support/delegates-pr/topic/rules-guidelines/
                url = 'https://lwf.support/delegates-pr/forum/delegate-promotions',
                nextPageRegex = /<li class="next"><a href.+? rel="next" role="button">/m,
                proposalRegex = /<a class="bbp-topic-permalink" href="https:\/\/lwf.support\/delegates-pr\/topic\/(.+?)">(.+?)<\/a>/mgi,
                result = [];
                ...
                function (next) {
                    ...
                    request.get({
                        ...
                        do {
                            ...
                            if (m && m[1] !== 'rules-guidelines\/') {
                                result.push ({
                                    proposal: m,
                                    topic: 'https:\/\/lwf.support\/delegates-pr\/topic\/' + m[1], 
