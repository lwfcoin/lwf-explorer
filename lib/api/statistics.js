...
module.exports = function (app) {
    ...
    this.getBlockTimes = function (error, success) {
        var blocks = new Blocks();

        request.get({
            url: app.get('lisk address') + blocks.url(0, 100),
            json : true
        }, function (err, resp, body) {
            if (err || resp.statusCode !== 200) {
                return error({ success : false, error : (err || 'Response was unsuccessful') });
            ...
            if (_.size(body.blocks) > 0) {
                ...
                for (let index = 0; index < body.blocks.length; index++) {
                    ...
                    result.push({
                        ...
                        timespan: (block.timestamp - aux.timestamp),
                        height: block.height,
    ...
    this.getTransactionsPerBlock = function (error, success) {
        var blocks = new Blocks();
        ...
        request.get({
            ...
            if (_.size(body.blocks) > 0) {
                body.blocks.reverse();                
                var result = body.blocks.map(block => {
                    return {
                        id: block.id,
                        height: block.height,
